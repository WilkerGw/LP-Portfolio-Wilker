    ...
    return (
        <section className="flex flex-col md:flex-row h-auto md:h-[600px] w-full bg-dark border-y border-white/5 overflow-hidden font-sans relative">
            ...
            <div className="flex flex-col md:flex-row flex-1 z-10 h-auto md:h-full">
              ...
              ${activeId === service.id ? 'h-[500px] md:h-full md:flex-[3]' : 'h-24 md:h-full md:flex-1'}`}
                    >
                        {/* Imagem de Fundo */}
                        <div
                        ...
                        <div className="relative h-full flex flex-col justify-end p-6 text-white">
                            ...
                            <h3 className={`text-2xl font-bold font-heading transition-all duration-500 ${activeId === service.id ? 'mb-4 translate-y-0 text-white' : 'mb-2 md:mb-10 md:-rotate-90 origin-left whitespace-nowrap text-gray-300'}`}>
                                {service.title}
                            </h3>

                            {/* Descrição e Botão (SÓ APARECEM NO HOVER) */}
                            <div className={`overflow-hidden transition-all duration-500 ease-in-out ${activeId === service.id ? 'max-h-[300px] opacity-100' : 'max-h-0 opacity-0'}`}>
